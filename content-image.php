...
if ( is_singular() && is_main_query() ) {
	...
	if( of_get_option('blog_single_thumbnail', '1') == '1' && has_post_thumbnail() && of_get_option( 'single_thumb', '1' ) == '1' ) { ?>
		<div id="post-thumbnail">
			<img src="<?php echo aq_resize( wp_get_attachment_url( get_post_thumbnail_id() ),  wpex_img( 'post_width' ), wpex_img( 'post_height' ), wpex_img( 'post_crop' ) ); ?>" alt="<?php wpex_esc_title(); ?>" />
		</div><!-- /post-thumbnail -->
	<?php }
	...
	<article <?php post_class('post-entry grid-3 clearfix'); ?>>
		<?php if( has_post_thumbnail() ) {  ?>
			<div class="post-entry-thumbnail">
				<a href="<?php echo wp_get_attachment_url( get_post_thumbnail_id() ); ?>" title="<?php wpex_esc_title(); ?>" class="fancybox">
